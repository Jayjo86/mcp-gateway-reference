...
audience URI: `urlsplit("api://mcp-server-b").netloc` is "mcp-server-b", not
...
REQUIRED_ENV = {
    ...
    "MCP_SERVER_B_AUDIENCE": "api://mcp-server-b",
    "MCP_SERVER_B_APP_ID": "bbbbbbbb-2222-3333-4444-555555555555",
    "MCP_RESOURCE_URL": "http://localhost:9002/mcp",
}
...
def test_verifier_accepts_both_the_audience_uri_and_the_app_id(
    ...
    assert provider.token_verifier.audience == [
        REQUIRED_ENV["MCP_SERVER_B_AUDIENCE"],
        REQUIRED_ENV["MCP_SERVER_B_APP_ID"],
    ]
...
def test_app_id_is_not_guessed_from_the_short_form_audience(
    ...
    """A short-form audience makes urlsplit(...).netloc == "mcp-server-b".
    That string is not a GUID and must never appear as an accepted audience —
    only the explicit MCP_SERVER_B_APP_ID may.
    """
    _set_env(monkeypatch)
    provider = build_auth()
    assert "mcp-server-b" not in provider.token_verifier.audience
    assert REQUIRED_ENV["MCP_SERVER_B_APP_ID"] in provider.token_verifier.audience


def test_missing_app_id_fails_fast(monkeypatch: pytest.MonkeyPatch) -> None:
    _set_env(monkeypatch, MCP_SERVER_B_APP_ID="")
    with pytest.raises(RuntimeError, match="MCP_SERVER_B_APP_ID"):
        build_auth()
