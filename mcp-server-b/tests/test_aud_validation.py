"""Audience validation on mcp-server-b.

The foundational invariant: a token issued for api://mcp-server-a is
rejected by mcp-server-b's JWTVerifier, and vice versa. Uses a locally
...
@pytest.mark.asyncio
async def test_server_a_token_rejected(key: RSAKeyPair, verifier: JWTVerifier) -> None:
    """The core invariant: a token for server-a cannot authenticate to server-b."""
...
async def test_expired_token_rejected(key: RSAKeyPair, verifier: JWTVerifier) -> None:
    token = key.create_token(subject="u1", issuer=ISSUER, audience=AUD_B, expires_in_seconds=-1)
    assert await verifier.load_access_token(token) is None


@pytest.mark.asyncio
async def test_wrong_issuer_rejected(key: RSAKeyPair, verifier: JWTVerifier) -> None:
    token = key.create_token(subject="u1", issuer="https://evil.example.invalid", audience=AUD_B)
    assert await verifier.load_access_token(token) is None
...
async def test_v1_sts_issuer_rejected(key: RSAKeyPair) -> None:
    ...
    v1_token = key.create_token(
        subject="u1", issuer=f"https://sts.windows.net/{tenant}/", audience=AUD_B
    )
